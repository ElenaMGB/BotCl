...
async def process_schedule(message: types.Message):
    ...
    try:
        ...
        # Сохраняем урок, проверяя на дубликаты
        if user_id not in lessons:
            lessons[user_id] = []

        lesson_time_str = lesson_time.strftime('%Y-%m-%d %H:%M')

        if lesson_time_str in lessons[user_id]:
            await message.answer("Этот урок уже запланирован.")
            return

        lessons[user_id].append(lesson_time_str)  # Сохраняем в строковом формате
...
async def cmd_lessons(message: types.Message):
    ...
    logging.info(f"Загруженные уроки для пользователя ID {user_id}: {user_lessons}")
